☁️ Cloud Engineering Learning Journey

Structured 244-day roadmap to become a Cloud Engineer

Hi, I'm Marvan Faizal CH.

I'm learning Cloud Engineering from scratch through a strict, hands-on, portfolio-driven roadmap. This repository documents my daily learning, labs, notes, projects, and progress as I work toward becoming a job-ready Cloud Engineer.

---

🎯 Goals

- Master Linux, Networking, AWS, Azure, Docker, Kubernetes & Terraform
- Build real-world projects that recruiters can actually test
- Earn industry-recognized certifications (AWS SAA-C03 + AZ-104)
- Create a strong, transparent GitHub portfolio
- Land a Cloud Support / Cloud Operations role in the UAE

---

📅 Current Status

| Status | Details |
|---|---|
| Day | Day 7 |
| Phase | Phase 1 – Networking Foundations |
| Last Updated | 23 July 2026 |
| Next Milestone | Complete Week 1 Networking Labs |

---

🗂️ Repository Structure

Cloud-Engineering-Journey/

- Notes/
  - Week1/
    - [Day-1.md](notes/Day-1.md)
    - [Day-2.md](notes/Day-2.md)
    - [Day-3.md](notes/Day-3.md)
    - [Day-4.md](notes/Day-4.md)
    - [Day-5.md](notes/Day-5.md)
    - [Day-6.md](notes/Day-6.md)
    - [Day-7.md](notes/Day-7.md)
  - Week2/
    - [Day-8.md](notes/Day-8.md)
    - [Day-9.md](notes/Day-9.md)

- projects/
  - (hands-on builds will go here as I progress)
- README.md

---

🛠️ Tech Stack Covered

`Linux` `Networking` `Git & GitHub` `AWS` `Azure` `Docker` `Kubernetes` `Terraform` `Python`

---

📈 Progress Log

| Week | Focus | Status |
|---|---|---|
| Week 1 | Networking Foundations & OSI Model | ✅ Completed|
| Week 2 | IP Addressing & Subnetting | 🔄 In Progress |
| Week 3 | Protocols & Routing | ⏳ Not Started |
| Week 4 | Cloud Networking Basics | ⏳ Not Started |

---

📫 Connect

Feel free to follow along as I document this journey day by day.
